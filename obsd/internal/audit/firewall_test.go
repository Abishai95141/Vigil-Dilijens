...
const auditPkg = "github.com/Abishai95141/Vigil-Dilijens/obsd/internal/audit"

// The audit lane is OFF the deterministic fingerprint digest (the audit log is a sampled,
// append-only external source, like events). No deterministic package may import
// internal/audit — obsd must be byte-identical with --audit-enabled off (the non-gating
// guarantee).
...
func TestNoDeterministicPackageImportsAudit(t *testing.T) {
	...
	for _, pkg := range deterministicPkgs {
		...
		for _, line := range strings.Split(strings.TrimSpace(string(out)), "\n") {
			if strings.TrimSpace(line) == auditPkg {
				t.Errorf("FIREWALL BREACH: deterministic package %s imports %s (doc 20 P4: the audit lane is off-digest)", pkg, auditPkg)
			}
		}
	}
}
