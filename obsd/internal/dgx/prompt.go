...
2. NO CAUSATION (with ONE gated exception): A correlation or co-occurrence is association, NOT cause. For "X is related to Y" use kind "edge" with relation "associated-with" (or "topology"/"topo-adjacent"). For a temporal "X co-occurs with / precedes Y" hunch use kind "causal_hypothesis" with relation "co-occurrence" — it is direction-free and is NEVER a cause. An edge using a causal relation ("causes", "leads to", ...) is discarded.
   DIRECTION (doc 33): on a "causal_hypothesis" you MAY add a "direction" — one of "a-to-b", "b-to-a", or "not-causal". A POSITIVE direction ("a-to-b"/"b-to-a") is allowed ONLY when you cite a get_causal_hypotheses observation (cohyp:<id>) whose detail says the lead-lag "AGREES with onset order"; otherwise leave it direction-free. "not-causal" (you believe the pair co-occurs but is NOT causal — e.g. a shared confounder, or the lead-lag DISAGREES with the onset order) may be set freely. Every direction is a SUGGESTION ONLY: a human authors or rejects it; the system never authors it.
...
{"proposals":[{"kind":"node|edge|member|bar_source|causal_hypothesis|equiv_group","subject":"short id","relation":"associated-with|topology|topo-adjacent|co-occurrence|","direction":"a-to-b|b-to-a|not-causal","group":"EQG_…","pattern":"^…$","canonical":"…","label":"…","evidence":["ref","ref"],"rationale":"short, non-causal note"}]}
The group/pattern/canonical/label fields are used ONLY for kind "equiv_group"; the direction field ONLY for a witness-agreeing causal_hypothesis; omit them otherwise. If nothing is well-grounded, return {"proposals":[]}.`
