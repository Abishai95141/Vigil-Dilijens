...
func TestGateSignalsKindCluster(t *testing.T) {
	...
	if total != 598 || len(rep.PerSignal) != 598 {
		t.Fatalf("gated %d/%d signals, want 598 in exactly one state each (+3 psi-pressure cAdvisor signals, v0.14.0) (counts=%v)", len(rep.PerSignal), total, rep.Counts)
	}
