...
func TestPhenomenonObservabilityKind(t *testing.T) {
	...
	if got := rep.Full + rep.Partial + rep.None; got != 47 || len(rep.PerPhenomenon) != 47 {
		t.Fatalf("verdicts = %d (full %d partial %d none %d), want 47 (+ 3 PSI saturation, v0.14.0; + PHEN_PVC_FILLING none on kind: volume-stats out-of-scope when the kubelet lane is off, v0.15.0)", got, rep.Full, rep.Partial, rep.None)
	}
