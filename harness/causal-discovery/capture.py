...
def scrape_cadvisor(node, metrics, namespace):
    ...
    for line in r.stdout.splitlines():
        ...
        if "_network_" in name:
            # cAdvisor network counters are POD-level (container=""); keep the pod aggregate
            # (prefer eth0) rather than dropping them with the pod-cgroup roll-up.
            iface = lbl.get("interface", "")
            if container != "" or iface not in ("", "eth0"):
                continue
            container = "pod"
        elif container in ("", "POD"):
            # skip pod-cgroup roll-up "" and the pause "POD" for per-container metrics
            continue
        pod = lbl.get("pod", "")
        try:
