...
- [~] **07 M2–M6** — **M2 DONE: first-order spans with the edge-validity contract.** Matcher evaluates spanned phenomena at their AUTHORED anchor only; neighbour members across declared edge types one hop (Container anchors walk from their pod — containment is identity, not a hop); absent edge never contributes (degrade-never-fabricate, golden-tested incl. window-intersection semantics: retracted-inside-window supports, retracted-before/asserted-after = absent); suspect edge degrades + is NAMED; findings carry span instantiation (edge path + validity verdicts + per-neighbour evidence rows). **Anchor-evidence rule (live-caught): a match requires ≥1 required member met ON THE ANCHOR — neighbour evidence corroborates, never alone constitutes** (before: every container on a pressured node fired "degraded cascade" on node PSI alone, incl. limit-less hogs where throttling is structurally impossible). **Live-verified on kind (chaos corpus `corpus/chaos/cpu-cascade.yaml`): throttle-victim (100m limit, spinning) + 6 unlimited hogs on worker2 → PHEN_THROTTLING_CASCADE FULL match 100% at the victim — ratio well-above (anchor) + node PSI above (neighbour) across a VALID runs-on edge, span path in the derivation; hogs/coredns suppressed (zero anchor evidence); healthy cluster SILENT (PSI observable-and-not-met fails the conjunction — §3.5 noise suppression live); kindnet/node-exporter cascades during the hog run were TRUE positives (shared-VM cores: real throttling + real node pressure, both members met).** M3 (second-order) / M4 (full degraded-match policy) / M5 (cascades+blast radius) / M6 (calibration) pending; second-order phenomena with checks are counted-skipped, stated.
...
- [ ] **07 M3–M6** Second-order spans; degraded-match policy completion; cascades + blast radius; sensitivity calibration
...
- [ ] **11 M3–M4** Falsification corpus v1 (chaos runs; `corpus/chaos/cpu-cascade.yaml` is item 1) + phenomenon precision/recall suite; sensitivity sweeps · **11 M2 carry-forward** binding-QA-over-fixtures + charter battery
